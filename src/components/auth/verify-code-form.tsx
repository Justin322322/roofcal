...
export default function VerifyCodeForm() {
  ...
  // Check rate limit status and restore cooldown state from localStorage
  useEffect(() => {
    const initializeOTP = async () => {
      if (email && !hasAutoGenerated) {
        setHasAutoGenerated(true);

        // Restore cooldown state from localStorage
        const storedCooldown = localStorage.getItem(`otp_cooldown_${email}`);
        if (storedCooldown) {
    ...
    initializeOTP();
  }, [email, session, hasAutoGenerated, startCooldownTimer]);
